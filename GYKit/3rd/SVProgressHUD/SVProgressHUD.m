...
@property (nonatomic, assign) BOOL showTipsOnly;
...
+ (void)setDismissCompletion:(SVProgressHUDDismissCompletion)dismissCompletion {
    [self sharedView].dismissCompletion = dismissCompletion;
}
...
+ (void)showTipsWithStatus:(NSString*)status {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _backgroundColor = [UIColor gy_colorWithRGB:0x000000 alpha:0.9f];
        _foregroundColor = [UIColor gy_colorWithRGB:0xDDDDDD];
        ...
        _font = [UIFont gy_CNFontWithFontSize:15];
        
        _infoImage = [[UIImage imageNamed:@"SVProgressHUD-info"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        _successImage = [[UIImage imageNamed:@"SVProgressHUD-success"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        _errorImage = [[UIImage imageNamed:@"SVProgressHUD-error"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (void)updateHUDFrame {
    ...
    CGFloat verticalSpacing = 12.0f; // |-x-content-(8-label-)x-|
    CGFloat horizontalSpacing = 12.0f; // |-x-content-x-|
    ...
    if(string) {
        const CGRect constraintRect = CGRectMake(0, 0, 200, 300);
        labelRect = [self.statusLabel textRectForBounds:constraintRect
                                 limitedToNumberOfLines:0];
        CGFloat labelHeight = ceilf(CGRectGetHeight(labelRect));
        CGFloat labelWidth = ceilf(CGRectGetWidth(labelRect));
        
        if (self.showTipsOnly) {
            hudHeight = labelHeight + 21;
        ...
        hudWidth = labelWidth + horizontalSpacing * 2;
    }
    
    // Update values on subviews
    const CGFloat hudViewHeight = self.showTipsOnly ? hudHeight : MAX(self.minimumSize.height, hudHeight);
    self.hudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth), hudViewHeight);
    ...
    // Label
    if (self.showTipsOnly) {
        labelRect.origin.y = roundf((hudHeight - labelRect.size.height) / 2);
    } else if(imageUsed || progressUsed){
    ...
    labelRect.origin.x = roundf((self.hudView.bounds.size.width - labelRect.size.width) / 2);
...
#else
    if (self.viewForExtension) {
        self.frame = self.viewForExtension.bounds;
    } else {
        ...
        if(strongSelf){
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.alpha == 0.0f && strongSelf.hudView.alpha == 0.0f){
                    ...
                    if (strongSelf.dismissCompletion) {
                        strongSelf.dismissCompletion();
                    }
                    strongSelf.dismissCompletion = nil;
...
- (void)setDismissCompletion:(SVProgressHUDDismissCompletion)dismissCompletion {
    if (!_isInitializing) _dismissCompletion = dismissCompletion;
}
