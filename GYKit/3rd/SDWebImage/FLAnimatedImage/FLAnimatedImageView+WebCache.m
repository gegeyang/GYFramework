...
+ (nullable UIImage *)sd_cachedImageForURLString:(nullable NSString *)urlString {
    if (urlString == nil || urlString.length == 0) {
        return nil;
    }
    
    UIImage *image = [SDWebImageManager.sharedManager cacheImageForURL:[NSURL URLWithString:urlString]];
    if (!image) {
        return nil;
    }
    ...
    if (image.sd_FLAnimatedImage) {
        return image;
    }
    NSString *key = [SDWebImageManager.sharedManager cacheKeyForURL:[NSURL URLWithString:urlString]];
    NSData *gifData = [[SDImageCache sharedImageCache] diskImageDataForKey:key];
    image.sd_FLAnimatedImage = [[FLAnimatedImage alloc] initWithAnimatedGIFData:gifData];
    return image;
}
